...
const Avatar = ({ name, role, size = 40 }: { name?: string; role?: string; size?: number }) => (
  <div className="rounded-full flex items-center justify-center font-bold text-white flex-shrink-0"
    style={{ width: size, height: size, fontSize: size * 0.38, background: AVATAR_BG[role || "STUDENT"] || "#6B7280" }}>
...
export default function StudentFeed() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setLoading(false);
      // Scroll to post if linked from notification
                      ...
                      {topComments.map((c) => {
                        ...
                        return (
                          <div key={c._id}>
                            ...
                            {/* Reply input inline under this comment */}
                            {isReplyingHere && renderReplyInput(c._id)}
                          </div>
                        );
                      })}
                    </div>
                  )}
                  {/* Main comment input (when not replying) */}
  ...
  return (
    <div className="min-h-screen" style={{ background: "#FFF8F5", fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* 3 columns layout */}
      <div style={{ padding: "24px 24px 24px 64px", marginTop: 16, display: "flex", gap: 40, justifyContent: "space-between" }}>

        {/* LEFT SIDEBAR */}
        <div style={{ width: 320, flexShrink: 0 }} className="hidden lg:block">
          <div className="sticky" style={{ top: 80 }}>
            {/* User */}
            <a href="/student/dashboard" className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-white transition" style={{ textDecoration: "none" }}>
        ...
        <div style={{ width: 320, flexShrink: 0 }} className="hidden xl:block">
          <div className="sticky" style={{ top: 80 }}>
            ...
            <div className="space-y-1">
              {[
                { icon: "calendar_today", label: "Lịch học hôm nay", href: "/student/schedule", color: "#3B82F6" },
                { icon: "assignment", label: "Báo cáo điểm danh", href: "/student/report", color: "#22C55E" },
                { icon: "face", label: "Đăng ký khuôn mặt", href: "/student/face-register", color: "#8B5CF6" },
                { icon: "event_available", label: "Xin phép nghỉ", href: "/student/leave-request", color: "#F59E0B" },
              ].map((item, i) => (
            ...
            <div className="space-y-2">
              {[
                { icon: "lightbulb", text: "Điểm danh đầy đủ giúp bạn đạt điều kiện dự thi dễ dàng hơn", color: "#F59E0B" },
                { icon: "trending_up", text: "Theo dõi báo cáo để biết số buổi vắng còn lại cho phép", color: "#22C55E" },
                { icon: "share", text: "Chia sẻ bài viết hữu ích để cùng nhau học tập tốt hơn", color: "#3B82F6" },
              ].map((tip, i) => (
